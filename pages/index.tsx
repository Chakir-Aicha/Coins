...
export default function Home() {
  ...
  const [selectedPage, setSelectedPage] = useState<'overview' | 'trends' | 'predictions'>('overview');

  const coinId = 'bitcoin';
  const increasePercentage = analysisResult ? analysisResult.increase_probability : 0;
  const decreasePercentage = analysisResult ? analysisResult.decrease_probability : 0;
  useEffect(() => {
    if (selectedPage === 'trends' && coin) {
      initializeChart(coin.id);
    }
    if (selectedPage === 'predictions') {
      updateChartFromCSV();
    }
  }, [selectedPage, coin]); // Déclenche la réinitialisation lorsque la page ou les données du coin changent
  ...
  useEffect(() => {
    async function fetchAnalysis() {
      ...
      try {
        ...
        // Extraire la partie JSON de la réponse Markdown (le bloc entre les backticks)
        const jsonResponse = data.response.replace(/```json\s+|\s+```/g, ''); // Retirer les backticks
        const analysisData = JSON.parse(jsonResponse); // Parser le JSON brut
    ...
    async function fetchCoin() {
      try {
        ...
        if (response.ok) {
          ...
          setCoin(data[0]);
          if (data[0]) {
            const adviceMessage = await fetchAdvice(data[0].current_price);
            if (adviceMessage) {
  ...
  async function fetchAdvice(currentPrice: number): Promise<string | null> {
    try {
      const response = await fetch('http://127.0.0.1:5000//gemini-advice', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch advice:', response.statusText);
      }
    } catch (error) {
      console.error('Error in advice API call:', error);
    }

    return null;
  }

  // Gestion de l'ouverture/fermeture de la fenêtre de chat
  const toggleChat = () => {
  ...
  async function initializeChart(coinId: string) {
    ...
    if (!ctx) {
      console.error(`Element with id "chart-${coinId}" not found.`);
      return;
    }
  ...
  return (
    <div className="dashboard-container">
      ...
      <aside className="dashboard-sidebar">
        <h2>Dashboard</h2>
        <ul>
          <li onClick={() => setSelectedPage('overview')} className={selectedPage === 'overview' ? 'active' : ''}>
            Overview
          </li>
          <li onClick={() => setSelectedPage('trends')} className={selectedPage === 'trends' ? 'active' : ''}>
            Market Trends
          </li>
          <li onClick={() => setSelectedPage('predictions')} className={selectedPage === 'predictions' ? 'active' : ''}>
            Predictions
          </li>
        </ul>
      </aside>
      ...
      <div className="dashboard-main">
        ...
        <div className="indicators">
          ...
          {selectedPage === 'predictions' && analysisResult && (
            <div>
            <div className="dashboard-analysis">
              <h2>Probability Analysis</h2>
              <ProgressBar>
                <ProgressBar
                  now={analysisResult.increase_probability}
                  label={`Increase: ${analysisResult.increase_probability}%`}
                  variant="success"
                  key={1}
                />
                <ProgressBar
                  now={analysisResult.decrease_probability}
                  label={`Decrease: ${analysisResult.decrease_probability}%`}
                  variant="danger"
                  key={2}
                />
              </ProgressBar>
